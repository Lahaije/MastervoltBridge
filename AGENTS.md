...
web_ui/ (web UI source and compiled assets)
  ├── web_ui_main.html  — main dashboard source (served at GET /)
  ├── web_ui_config.html — settings page source (served at GET /config)
  ├── web_ui.css — shared stylesheet source (served at GET /web_ui.css)
  ├── web_ui.js — shared JavaScript source (served at GET /web_ui.js)
  └── web_ui.h — PROGMEM asset blobs (generated by scripts/build_web_ui.py; do not edit manually)
...
`esp32_inverter_bridge.ino`, `wifi_bridge.{h,cpp}`, `inverter_data.{h,cpp}`, `inverter_link_state.{h,cpp}`, `inverter_controller.{h,cpp}`, `ethernet_bridge.{h,cpp}`, `api.{h,cpp}`, `api_helper.{h,cpp}`, `mqtt_client.{h,cpp}`, `mqtt_settings.{h,cpp}`, `web_ui/` (source files + generated `web_ui.h`), `settings.{h,cpp}`, `logger.h`

**Scripts** (`scripts/`):
`build_web_ui.py` — minifies `web_ui/` source files and packs into `web_ui/web_ui.h`; run after any web UI source change
