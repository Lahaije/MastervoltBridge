...
The current firmware release is `1.0.0`.

- `firmware/esp32_inverter_bridge/settings.cpp` defines `FIRMWARE_VERSION`
- `/api/device` and the web UI display the same value
- Update the version string in `settings.cpp` when a new release is cut
