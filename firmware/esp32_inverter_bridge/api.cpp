...
void handleGetRoot(EthernetClient& client) {
  sendFlashHtmlResponse(client, web_ui_main_html, web_ui_main_html_len);
}

void handleGetConfig(EthernetClient& client) {
  sendFlashHtmlResponse(client, web_ui_config_html, web_ui_config_html_len);
}

void handleGetWebUiCss(EthernetClient& client) {
  sendHttpResponse(client, 200, "text/css", String(web_ui_css).substring(0, web_ui_css_len));
}

void handleGetWebUiJs(EthernetClient& client) {
  sendHttpResponse(client, 200, "application/javascript", String(web_ui_js).substring(0, web_ui_js_len));
}
...
void handleApiClient(EthernetClient& client) {
  ...
  // Dispatch to appropriate handler based on method and path
  if (method == "GET" && path == "/") {
  ...
  if (method == "GET" && path == "/web_ui.css") {
    handleGetWebUiCss(client);
    return;
  }
  if (method == "GET" && path == "/web_ui.js") {
    handleGetWebUiJs(client);
    return;
  }
  if (method == "GET" && path == "/api") {
    handleGetApiDiscovery(client);
    return;
  }
  if (method == "GET" && path == "/api/health") {
    handleGetHealth(client);
    return;
  }
  if (method == "GET" && path == "/api/device") {
    handleGetDevice(client);
    return;
  }
  if (method == "GET" && path == "/api/logs") {
    handleGetLogs(client);
    return;
  }
  if (method == "GET" && path == "/pulse") {
    handleGetPulse(client);
    return;
  }
  if (method == "POST" && path == "/wifi/off") {
    handlePostWifiOff(client);
    return;
  }
  if (method == "POST" && path == "/api/debug") {
    handlePostDebug(client, body);
    return;
  }
  if (method == "POST" && path == "/api/interval") {
    handlePostInterval(client, body);
    return;
  }
  ...
  if (method == "POST" && path == "/api/power") {
    handlePostPower(client, body);
    return;
  }
  if (method == "POST" && path == "/api/shadow") {
    handlePostShadow(client, body);
    return;
  }
  if (method == "POST" && path == "/api/inverter/fetch") {
    handlePostInverterFetch(client, body);
    return;
  }
  if (method == "GET" && path == "/api/mqtt") {
    handleGetMqttSettings(client);
    return;
  }
  if (method == "POST" && path == "/api/mqtt") {
    handlePostMqttSettings(client, body);
    return;
  }
